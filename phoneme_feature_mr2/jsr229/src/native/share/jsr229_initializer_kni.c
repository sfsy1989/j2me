...
SUITESORE_DECLARE_LISTENER(jsr229_remove_listener);
 ...
 * com.sun.midp.jsr229.Initializer.
 * <p>
 * Finalizes the JSR229 subsystem.
 * Unregisters the callback on a midlet suite removal.
 */
KNIEXPORT KNI_RETURNTYPE_VOID
Java_com_sun_midp_jsr229_Initializer_cleanup() {
    (void) midp_suite_remove_listener(jsr229_remove_listener,
                                      SUITESTORE_LISTENER_TYPE_REMOVE);
    KNI_ReturnVoid();
}
 ...
 * is performed
 */
SUITESORE_DEFINE_LISTENER(jsr229_remove_listener) {
    (void) listenerType;
    (void) when;
    (void) status;
    (void) pSuiteData;
