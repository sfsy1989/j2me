...
JSR238_STATUSCODE jsr238_format_integer_number(jint locale_index, jlong l,
	...
	char* buf;
	int isnegative = (l<0);
	int res = javacall_mi18n_long_to_ascii((long)(isnegative?-l:l),&buf);
	if (res < 0) return res;
	return javacall_mi18n_format_number(locale_index, buf, 0, isnegative, 0, res_buffer, pbuffer_len);
}
...
JSR238_STATUSCODE jsr238_format_integer_percentage(jint locale_index, jlong l,
	...
	int res = javacall_mi18n_long_to_ascii((long)(isnegative?-l:l), &buf);
	if (res < 0) return res;
	return javacall_mi18n_format_percentage(locale_index, buf, 0, 0, isnegative, res_buffer, pbuffer_len);
}
