...
Java_com_sun_j2me_global_FormatAbstractionLayerImpl_getFormatLocalesCount() {
	int count=0;
	if (jsr238_get_format_locales_count(&count)<0){
		KNI_ThrowNew(midpRuntimeException,"Get locale count");
	}
    KNI_ReturnInt(count);
}
...
Java_com_sun_j2me_global_FormatAbstractionLayerImpl_getFormatLocaleName() {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_INT
Java_com_sun_j2me_global_FormatAbstractionLayerImpl_getFormatLocaleIndex() {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
Java_com_sun_j2me_global_FormatterImpl_formatDateTime0() {
	...
	if (res < 0) {
        ...
        if (NULL == (buffer = midpMalloc(len * sizeof(jchar)))) {
            KNI_ThrowNew(midpOutOfMemoryError, 
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
Java_com_sun_j2me_global_FormatterImpl_formatNumber0() {
    jint locale_index = KNI_GetParameterAsInt(1);
    ...
    if (res < 0) {
		KNI_ThrowNew(midpRuntimeException,"Internal error! Could not format number.");
		KNI_ReleaseHandle(hstr);
	} else {
        buffer = (jchar *)midpMalloc(res_len * sizeof(jchar)); 
        if (NULL == buffer) {
            KNI_ThrowNew(midpOutOfMemoryError, 
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
	Java_com_sun_j2me_global_FormatterImpl_formatNumber1() {
    jint locale_index = KNI_GetParameterAsInt(1);
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
Java_com_sun_j2me_global_FormatterImpl_formatCurrency0() {
    jint locale_index = KNI_GetParameterAsInt(1);
	jdouble d = KNI_GetParameterAsDouble(2);
	...
	if (code_len>=0) {
		code = (jchar *)midpMalloc((code_len +1)  * sizeof(jchar));
		if (NULL == code) {
			error = 1;
			KNI_ThrowNew(midpOutOfMemoryError, 
	...
	if (!error){
		...
		if (res < 0) {
			...
			buffer = midpMalloc(res_len * sizeof(jchar)); 
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
Java_com_sun_j2me_global_FormatterImpl_formatPercentage0() {
    jint locale_index = KNI_GetParameterAsInt(1);
    ...
    if (res < 0) {
		KNI_ThrowNew(midpRuntimeException,"Could not format percentage value");
		KNI_ReleaseHandle(hstr);
	} else {
        buffer = (jchar *)midpMalloc(res_len * sizeof(jchar)); 
        if (NULL == buffer) {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_OBJECT 
Java_com_sun_j2me_global_FormatterImpl_formatPercentage1() {
    jint locale_index = KNI_GetParameterAsInt(1);
    jlong l = KNI_GetParameterAsLong(2);
    jsize res_len = 0, res;
    jchar *buffer;
    ...
    if (res < 0) {
		KNI_ThrowNew(midpRuntimeException,"Could not format number");
		KNI_ReleaseHandle(hstr);
	} else {
        buffer = (jchar *)midpMalloc(res_len * sizeof(jchar)); 
        if (NULL == buffer) {
