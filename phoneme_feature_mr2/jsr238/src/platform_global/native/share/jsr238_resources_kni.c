...
Java_com_sun_j2me_global_DevResourceManagerFactory_getDevLocalesCount() {
	int count=0;
...
Java_com_sun_j2me_global_DevResourceManagerFactory_getDevLocaleName() {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_INT
Java_com_sun_j2me_global_DevResourceManagerFactory_getDevLocaleIndex() {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_INT
Java_com_sun_j2me_global_DevResourceBundleReader_getRawResourceData0() {
	...
	if (array_len < length){
        length = 0;
        KNI_ThrowNew(midpArrayIndexOutOfBoundsException, 
            "Error! Array size is too few!");
	} else {
		buffer = midpMalloc(array_len);
		if (NULL == buffer) {
			length = 0;
			KNI_ThrowNew(midpOutOfMemoryError, 
 ...
 */
KNIEXPORT KNI_RETURNTYPE_INT
Java_com_sun_j2me_global_DevResourceBundleReader_getResourceType0() {
	jint locale_index = KNI_GetParameterAsInt(1);
    jint resource_id = KNI_GetParameterAsInt(2);
    jint resType;
...
Java_com_sun_j2me_global_DevResourceBundleReader_isValidResourceID0() {
    jint locale_index = KNI_GetParameterAsInt(1);
    jint resource_id = KNI_GetParameterAsInt(2);
	jboolean res = 0;
	if (JSR238_STATUSCODE_OK != jsr238_is_valid_resource_id(locale_index, resource_id, &res)){
		KNI_ThrowNew(midpRuntimeException,"error getting isValidResourceID");
	}
 ...
 */
KNIEXPORT KNI_RETURNTYPE_INT
Java_com_sun_j2me_global_DevResourceBundleReader_getResourceLength0() {
    jint locale_index = KNI_GetParameterAsInt(1);
    jint resource_id = KNI_GetParameterAsInt(2);
    jsize resLength;
