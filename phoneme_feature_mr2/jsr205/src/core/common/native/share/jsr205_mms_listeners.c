...
void jsr205_mms_message_arrival_notifier(MmsMessage* message) {
    ...
    if(mms_midlet_listeners != NULL) {
        unblocked = invokeMMSListeners(message, mms_midlet_listeners);
    }
    ...
    if ((unblocked == WMA_ERR) && (mms_push_listeners != NULL)) {
        unblocked = invokeMMSListeners(message, mms_push_listeners);
    }
...
static WMA_STATUS jsr205_mms_midlet_listener(MmsMessage* message, void* userData)
{
    ...
    return jsr205_mms_unblock_thread((int)userData, WMA_MMS_READ_SIGNAL);
