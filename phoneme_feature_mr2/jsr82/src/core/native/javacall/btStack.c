...
{
    LOG("javacall_bt_stack_initialize()");
    ensureInitialized();
    return JAVACALL_OK;
}
...
{
    initDevice();
    memcpy(pAddr, emul_data.local_addr, BT_ADDRESS_SIZE);
    return JAVACALL_OK;
}
...
{
    memcpy(pName, deviceName, strlen(deviceName) + 1);
    return JAVACALL_OK;
}
 ...
 * @retval <code>JAVACALL_FAIL</code> on failure
 */
javacall_result javacall_bt_stack_get_acl_handle(
 ...
 *
 * Whenever a device is discovered it is notified via
 * javanotify_bt_device_discovered().
 *
 * The notification of inquiry completion will be sent via
 * <code>javanotify_bt_event()</code>
 * with <code>JAVACALL_EVENT_BT_INQUIRY_COMPLETE</code> type. 
 *
...
{
    LOG("javacall_bt_stack_start_inquiry()");
    START_REQUEST
...
{
    LOG("javacall_bt_stack_cancel_inquiry()");
    START_REQUEST
...
javacall_result javacall_bt_stack_ask_friendly_name(
        const javacall_bt_address addr)
{
    LOG("javacall_bt_stack_ask_friendly_name()");
...
javacall_result javacall_bt_stack_authenticate(
        const javacall_bt_address addr)
{
    LOG("javacall_bt_stack_authenticate()");
