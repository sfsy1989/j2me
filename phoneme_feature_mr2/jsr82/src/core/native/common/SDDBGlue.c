...
Java_com_sun_kvem_jsr082_bluetooth_SDDB_updateRecord(void)
{
    ...
    if (bt_sddb_update_record(&record) == BT_RESULT_SUCCESS) {
        retval = record.id;
...
{
    bt_sddb_remove_record((bt_sddbid_t)KNI_GetParameterAsInt(1));
    bt_stack_set_service_classes(bt_sddb_get_service_classes(0));
    KNI_ReturnVoid();
}
...
Java_com_sun_kvem_jsr082_bluetooth_SDDB_readRecord(void)
{
    ...
    if (bt_sddb_read_record(&record) == BT_RESULT_SUCCESS) {
        retval = record.size;
    } else {
        retval = 0;
    }
...
{
    KNI_ReturnInt(bt_sddb_get_service_classes(KNI_GetParameterAsInt(1)));
}
...
Java_com_sun_kvem_jsr082_bluetooth_SDDB_getRecords(void)
{
    ...
    if (KNI_IsNullHandle(arrayHandle)) {
        retval = bt_sddb_get_records(NULL, 0);
    } else {
        retval = bt_sddb_get_records((bt_sddbid_t *)JavaIntArray(arrayHandle),
