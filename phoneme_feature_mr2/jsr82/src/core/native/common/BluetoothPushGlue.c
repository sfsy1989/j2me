...
Java_com_sun_midp_jsr082_bluetooth_BluetoothPush_registerUrl(void)
{
    ...
    if (bt_push_register_url(szUrl, data, size) == BT_RESULT_SUCCESS) {
        retval = KNI_TRUE;
    }
    midpFree(szUrl);
...
Java_com_sun_midp_jsr082_bluetooth_BluetoothPush_getRecordHandle(void)
{
    jint retval = 0;
    MidpString wsUrl;
    char *szUrl;
