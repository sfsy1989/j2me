...
public class L2CAPNotifierImpl extends BluetoothNotifier
    ...
    private boolean isListenMode = false;
     ...
     */
    L2CAPNotifierImpl(BluetoothUrl url, int mode) throws IOException {
        super(url, mode);

        MIDletSuite suite = MIDletStateHandler.getMidletStateHandler().
                getMIDletSuite();

        int suiteId = (suite != null) ? suite.getID() :
     ...
     */
    private L2CAPConnection doAccept() throws IOException {
        if (!isListenMode) {
