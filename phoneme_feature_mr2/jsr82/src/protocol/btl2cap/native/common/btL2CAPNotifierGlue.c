...
*/
        ...
        if (status == BT_RESULT_FAILURE) {
            char* pError;
            bt_l2cap_get_error(peer, &pError);
            midp_snprintf(gKNIBuffer, KNI_BUFFER_SIZE,
                    "IO error in bt_l2cap_notif::finalize (%s)\n", pError);
            REPORT_ERROR(LC_PROTOCOL, gKNIBuffer);
        } else if (status == BT_RESULT_WOULDBLOCK) {
...
Java_com_sun_midp_io_j2me_btl2cap_L2CAPNotifierImpl_create0(void) {
    ...
    bt_bool_t authz  = (KNI_GetParameterAsBoolean(4) == KNI_TRUE)
        ? BT_BOOL_TRUE : BT_BOOL_FALSE;
    bt_bool_t enc  = (KNI_GetParameterAsBoolean(5) == KNI_TRUE)
        ? BT_BOOL_TRUE : BT_BOOL_FALSE;
    bt_bool_t master  = (KNI_GetParameterAsBoolean(6) == KNI_TRUE)
        ? BT_BOOL_TRUE : BT_BOOL_FALSE;
...
*/
    ...
    if (bt_l2cap_create_server(imtu, omtu, auth, authz, enc, master,
            &handle, &psm) == BT_RESULT_FAILURE) {
...
Java_com_sun_midp_io_j2me_btl2cap_L2CAPNotifierImpl_listen0(void) {
    ...
    if (KNI_GetIntField(thisHandle, pushHandleID) == BT_INVALID_PUSH_HANDLE) {
        ...
        /* force listening */
        if (bt_l2cap_listen(handle) == BT_RESULT_FAILURE) {
...
*/
//            } else {
                status = bt_l2cap_accept_start(
                    handle, &peer_handle, peer_addr, &imtu, &omtu, &context);
                processStatus = KNI_TRUE;
//            }
...
*/
                ...
                RegisterL2CAPConnection0(peer_addr, peer_handle);
...
*/
    ...
    if (peer != BT_INVALID_HANDLE) {
        if (bt_l2cap_close(peer) == BT_RESULT_FAILURE) {
            REPORT_ERROR(LC_PROTOCOL,
