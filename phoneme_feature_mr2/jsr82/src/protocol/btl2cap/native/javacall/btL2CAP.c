 ...
 */
/*OPTIONAL*/ javacall_result javacall_bt_l2cap_get_error(javacall_handle handle,
    /*OUT*/ char** pErrStr)
{
    static char* empty = "";
    (void)handle;
...
{
    LOG1("javacall_bt_l2cap_close(%d)", (int)handle);
...
static void checkAuthEnc(javacall_handle handle) {
    ...
    if (info->flags & AUTHENTICATE) {
        javacall_bt_bcc_bond(info->peer_addr, fakePin, &fakeRetval);
    }
    if (info->flags & ENCRYPT) {
        javacall_bt_bcc_set_encryption(info->peer_addr, JAVACALL_TRUE, &fakeRetval);
    }
}
 ...
 * @retval JAVACALL_FAIL on failure
 */
javacall_result javacall_bt_l2cap_create_server(int imtu, int omtu, javacall_bool auth,
    javacall_bool authz, javacall_bool enc, javacall_bool master, 
    /*OUT*/ javacall_handle* pHandle, /*OUT*/ int* pPsm)
{

    LOG("javacall_bt_l2cap_create_server()");
        ...
        /*OUT*/int *pTransmitMTU)
{
	...
	if (info->is_accepting == JAVACALL_FALSE) {
        ...
        LOG1("javacall_bt_l2cap_accept_start(%d)", (int)handle);
        ...
        // listen()
        status = javacall_bt_l2cap_accept(handle, pPeerHandle, pPeerAddr,
            pReceiveMTU, pTransmitMTU);
	} else { /* finish */
        LOG("javacall_bt_l2cap_accept_finish()");
        ...
        if (status == JAVACALL_OK) {
            ...
            *pPeerHandle = (javacall_handle) info->conn_handle;
            ...
            if (BT_INVALID_PUSH_HANDLE != bt_push_find_server(handle)) {
                javacall_bt_l2cap_listen(handle);
            }
        }
	}
    return status;
 ...
 * @retval JAVACALL_FAIL otherwise
 */
javacall_result javacall_bt_l2cap_create_client(int imtu, int omtu, javacall_bool auth,
    javacall_bool enc, javacall_bool master, /*OUT*/ javacall_handle* pHandle)
{

    LOG("javacall_bt_l2cap_create_client()");
 ...
 * @retval JAVACALL_FAIL otherwise
 */
javacall_result javacall_bt_l2cap_send(javacall_handle handle,
 ...
 * @retval JAVACALL_FAIL otherwise
 */
javacall_result javacall_bt_l2cap_receive(javacall_handle handle, /*OUT*/ char *pData,
 ...
 * @retval JAVACALL_FAIL on failure
 */
javacall_result javacall_bt_bcc_set_encryption(const javacall_bt_address addr, javacall_bool enable,
        javacall_bool *pBool)
{
    int handle = getConnHandle(addr);
    *pBool = JAVACALL_FALSE;
 ...
 * @retval JAVACALL_WOULD_BLOCK if the caller needs a notification to complete the operation
 */
javacall_result javacall_bt_bcc_bond(const javacall_bt_address addr, const char* pin,
        javacall_bool *pBool)
{
    int handle = getConnHandle(addr);
