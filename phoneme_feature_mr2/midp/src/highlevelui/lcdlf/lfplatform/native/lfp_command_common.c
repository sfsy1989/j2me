 ...
 */
MidpCommand* MidpCommandSortAll(kjobject ItemCmdArray, int numItemCmds,
				kjobject DispCmdArray, int numDispCmds) {
    ...
    kjobjectArray  itemCmds = (kjobjectArray)ItemCmdArray;
    kjobjectArray  cmds = (kjobjectArray)DispCmdArray;
    ...
    for (j = 0; j < nc; ++j) {
        ...
        if(PCSL_STRING_OK
                != midp_kjstring_to_pcsl_string(str, &c[j].shortLabel_str)) {
            break;
        }

        str = hand(getMidpCommandPtr(i)->longLabel);
        if(PCSL_STRING_OK
                != midp_kjstring_to_pcsl_string(str, &c[j].longLabel_str)) {
