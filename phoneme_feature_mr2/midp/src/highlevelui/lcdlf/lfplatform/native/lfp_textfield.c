...
/** Get a DynamicCharacterArray structure from the given <tt>kjobject</tt> */
...
Java_javax_microedition_lcdui_TextFieldLFImpl_createNativeResource0() {
    ...
    if (PCSL_STRING_OK ==
       midp_jchar_array_to_pcsl_string(bufferJCharArray, textLength, &text)) {
        if (PCSL_STRING_OK ==
           midp_kjstring_to_pcsl_string(labelJString, &label)) {
            if (PCSL_STRING_OK ==
               midp_kjstring_to_pcsl_string(inputModeJString, &inputMode)) {
                err = KNI_OK;
            }
        }
    }
