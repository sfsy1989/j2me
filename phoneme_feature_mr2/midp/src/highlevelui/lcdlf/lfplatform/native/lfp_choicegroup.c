...
Java_javax_microedition_lcdui_ChoiceGroupLFImpl_createNativeResource0() {
  ...
  if (numChoices > 0) {
    ...
    for (i = 0; err == KNI_OK && i < numChoices; i++) {
      ...
      perr = midp_kjstring_to_pcsl_string(strJString, &cgChoices[i].string);
  ...
  if (err == KNI_OK) {
    if(PCSL_STRING_OK
        != midp_kjstring_to_pcsl_string(labelJString, &label_str)) {
      err = KNI_ENOMEM;
    }
  }
...
Java_javax_microedition_lcdui_ChoiceGroupLFImpl_insert0() {
  ...
  perr = midp_kjstring_to_pcsl_string(stringPartJString, &cgElement.string);
  ...
  if (selectedArray == NULL) {
    ...
    for (i = 0; i < selectedArrayLength; i++) {
      selectedArray[i] = KNI_GetBooleanArrayElement(
				    (kjbooleanArray)selectedArrayJObjectArray, 
				    (jsize)i);
    }
