 ...
 */
 #ifdef FV_USE_QT4
TextViewer::TextViewer(QWidget* parent) : QTextEdit(parent) {
 #else
...
#else
    : QWidget(parent, NULL, WType_TopLevel | WStyle_Customize | WStyle_NoBorder) {
#endif
    imageHolder = NULL;
    textViewer = NULL;
    numButtons = 0;
            ...
            || numOfCmds == ALERT_NUM_OF_BUTTONS) {
                ...
                GET_PCSL_STRING_DATA_AND_LENGTH(short_label) {
                    if(PCSL_STRING_PARAMETER_ERROR(short_label)) {
                        ...
                        #ifdef FV_USE_QT4
                        label.setUtf16(short_label_data,
                                       short_label_len);
                        #else
...
alert_set_title(MidpDisplayable* alertPtr, const pcsl_string* title) {
    ...
    #ifdef FV_USE_QT4
    ((Alert *)alertPtr->frame.widgetPtr)->setWindowTitle(qtitle);
    #else
