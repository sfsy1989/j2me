 ...
 */
//#ifdef FV_USE_QT4
class PlatformMScreen : public QFrame, public MScreen {
//#else
...
private:
    ...
    int gc_type;
	int gc_flag;
	QPixmap *mscreen_pixmap;
	QImage mscreen_image;
	int gc_x;
    int gc_y;
    int gc_width;
    int gc_height;

    QImage old_mscreen_image;
	int old_gc_x;
    int old_gc_y;
    int old_gc_width;
    int old_gc_height;
...
public:
    int setupGCParas(int pixel_pen, int pixel_brush, const jshort *clip,
		      QPaintDevice *dst, int dotted,int x, int y, int width, int height);
	int setGCType(int gc_t);
	void gcUpdate();
	void setupImage(QImage imagePtr);
