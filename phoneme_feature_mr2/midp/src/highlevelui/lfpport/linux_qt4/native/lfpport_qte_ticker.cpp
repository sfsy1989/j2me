...
#ifdef FV_USE_QT4
  : QFrame( parent), offset( 5 ), speed( 0 ), txt( "Ready" )
#else
...
void Ticker::drawContents( QPainter * p ) {
  ...
  if (speed < 1) {
      ...
      p->drawText( 2, 0, INT_MAX, height(), Qt::AlignVCenter, txt );
      return;
  }
  ...
  for ( int x = 0; x < cachedWidth - offset; x += fw )
      p->drawText( x + offset, 0, INT_MAX, cachedHeight, Qt::AlignVCenter, txt );
