 ...
 */
ChameleonMScreen::ChameleonMScreen(QWidget *parent, const char* name)
{
	(void)parent;
	(void)name;
    vm_stopped = false;
...
#ifdef FV_USE_QT4
		qpixmap.copy(0,0,getDisplayFullWidth(), getDisplayFullHeight());
#else
        qpixmap.resize(getDisplayFullWidth(), getDisplayFullHeight());
#endif
    } else {
#ifdef FV_USE_QT4
		qpixmap.copy(0,0,getDisplayWidth(), getDisplayHeight());
#else
...
void ChameleonMScreen::paintEvent(QPaintEvent *pEvent)
{
    //QRect r(e->rect());
...
int ChameleonMScreen::setupGCParas(int pixel_pen, int pixel_brush, const jshort *clip,
...
int ChameleonMScreen::setGCType(int gc_t){
		(void)gc_t;
		return 0;
}

void ChameleonMScreen::gcUpdate() {

}

void ChameleonMScreen::setupImage(QImage imagePtr){
	(void)imagePtr;
}
