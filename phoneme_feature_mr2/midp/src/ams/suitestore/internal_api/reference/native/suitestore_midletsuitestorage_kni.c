...
KNIDECL(com_sun_midp_midletsuite_MIDletSuiteStorage_getMidletSuiteJarPath) {
    ...
    do {
        ...
        midp_kjstring_from_pcsl_string(KNIPASSARGS &classPath, tempHandle);
    } while (0);
 ...
 */
static void fillAuthPath(KNIDECLARGS MidpInstallInfo src, kjobject dst, kjclass dstClass) {
    int i;
 ...
 */
static MIDPError fillVerifyHash(SuiteIdType suiteId, kjobject installInfoObj) {
    int len;
    jbyte* data = NULL;
    jfieldID fieldID;
    MIDPError status;
...
KNIDECL(com_sun_midp_midletsuite_SuiteProperties_load) {
    ...
    do {
        ...
        if (error != MIDP_ERROR_NONE) {
            openError = 1;
			printf("if (error != MIDP_ERROR_NONE)\n");
            break;
        }
        ...
        if (KNI_IsNullHandle(properties)) {
			...
			printf("error = MIDP_ERROR_OUT_MEM;\n");
            break;
        }

		printf("numProperties is %d\n",numProperties);
        for (i = 0; i < numProperties*2; i+=2) {
            ...
            if (error == MIDP_ERROR_NONE) {
                ...
                midpFree(key_data);
                midpFree(value_data);
				printf("error while read properties\n");
                break;
          }
        }
    } while(0);
    ...
    switch(error) {
        case MIDP_ERROR_OUT_MEM:
			printf("case MIDP_ERROR_OUT_MEM\n");
            KNI_ThrowNew(midpOutOfMemoryError, NULL);
            break;
...
#define GET_PROP_PARAM(NUM, PARAM, STRINGOBJ, PROPS, STATUS) { \
 ...
 \
    ...
    for (i = 0; i < numberOfStrings; i++) { \
        ...
        if (PCSL_STRING_OK != midp_kjstring_to_pcsl_string((STRINGOBJ), \
                &(PROPS.pStringArr[i]))) { \
            int j; \
            for (j = 0; j < i; j++) { \
        ...
        if (installInfo.authPathLen > 0) {
            ...
            for (i = 0; i < installInfo.authPathLen; i++) {
                ...
                if (KNI_GetStringLength(tmpHandle) >= 0) {
                    if (midp_kjstring_to_pcsl_string(tmpHandle,
