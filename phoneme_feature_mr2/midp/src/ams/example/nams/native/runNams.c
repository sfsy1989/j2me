...
#define JVM_STOP_STATUS  "1"
#define JVM_STATUS_FILE  "/var/jvm_status"
static void setMidletStatus(char * status)
{
	int fd=-1;
	char sBuf[8];
	...
	strncpy(sBuf,status,1);
	if((fd=open(JVM_STATUS_FILE,O_RDWR|O_CREAT))!=-1){
...
void state_change_listener(const NamsEventData* pEventData) {
    ...
    if((pEventData->appId == 0) && (pEventData->state == MIDP_MIDLET_STATE_DESTROYED)){
        midp_system_stop();
		//setenv("jvm_stop", "1", 1);
		setMidletStatus(JVM_STOP_STATUS);
        exit(0);
    }
}
