 ...
 */
jfieldID midp_get_field_id(KNIDECLARGS kjclass classHandle,
                           const char* name,
 ...
 * @param signature the field's type
 */
jfieldID midp_get_static_field_id(KNIDECLARGS kjclass classHandle,
                                  const char* name,
                                  const char* signature);
 ...
 * @param fieldHandle handle where to put the resulting kjstring
 ...
 */
pcsl_string_status midp_get_string_field(KNIDECLARGS kjobject obj, kjclass classObj,
                                  char* pszFieldName, kjobject fieldHandle,
                                  pcsl_string* result);

/**
 * Set a kjobject field from Java platform native functions.
 *
 ...
 */
void midp_set_jobject_field(KNIDECLARGS kjobject obj,
 ...
 */
MidpString midpNewStringImpl(kjstring jStringHandle, char* filename, int line);
 ...
 * @return a new Unicode string
 */
MidpString midpNewStringFromArrayImpl(kjcharArray jCharArrayHandle, int length,
 ...
 * @return status of the operation
 */
pcsl_string_status midp_kjstring_to_pcsl_string(kjstring java_str,
 ...
 */
pcsl_string_status
midp_jchar_array_to_pcsl_string(kjcharArray java_arr, jint length,
                                pcsl_string * pcsl_str);
 ...
 * @return status of the operation
 */
pcsl_string_status midp_kjstring_from_pcsl_string(KNIDECLARGS const pcsl_string * pcsl_str,
						 kjstring java_str);
 ...
 */
jint midp_kjstring_to_address_and_length(kjstring jStringHandle, jchar* * pAddr);
...
#define GET_PARAMETER_AS_PCSL_STRING(index,id) \
    { \
        ...
        if(PCSL_STRING_OK != midp_kjstring_to_pcsl_string(id##_handle, &id)) { \
...
#define GET_JSTRING_AS_PCSL_STRING(handle,id) \
    { \
        pcsl_string id; \
        pcsl_string * const latest_pcsl_string_arg = &id; \
        if(PCSL_STRING_OK != midp_kjstring_to_pcsl_string(handle, &id)) { \
            KNI_ThrowNew(midpOutOfMemoryError, NULL); \
        } else { {
