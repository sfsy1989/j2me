 ...
 */
jboolean
midp_readJarEntry(const pcsl_string* jarName, const pcsl_string* entryName, kjobject* entry) {
    ...
    do {
        ...
        Jvm_read_jar_entry(platformJarName, (char*)platformEntryName, (kjobject)*entry);
        midpFree(platformJarName);
        midpFree(platformEntryName);
    } while(0);
    ...
    RELEASE_PCSL_STRING_DATA_AND_LENGTH
    if (noerror) {
        return (KNI_IsNullHandle((kjobject)*entry) == KNI_TRUE)
               ? KNI_FALSE : KNI_TRUE;
    } else {
        return KNI_FALSE;
    }
