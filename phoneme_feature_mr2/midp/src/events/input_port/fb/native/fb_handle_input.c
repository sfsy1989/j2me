...
static void init_key_device() {
    ...
    if (!done) {
        ...
        case LINUX_FB_FV:
			#ifdef ANDROID
			mapping = android_keys;
			#else
            mapping = fh2_keys;
			#endif
            bitscale_mode = KNI_FALSE;
            readKeyEvent = read_fh2_key_event;
            break;	
...
#endif
static void find_raw_key_mapping_fh2() {

    /* cache a few state fields for brewity */
    unsigned key = keyState.key;
	unsigned down = keyState.down;
    KeyMapping *km;

        /* key is key code */
        for (km = mapping; km->midp_keycode != KEYMAP_KEY_INVALID; km++) {
            if (km->raw_keydown == key) {
               if (km->raw_keyup == down) {
                keyState.down = 0;
                break;
               }else{
                keyState.down = 1;
