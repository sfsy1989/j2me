...
#ifdef FV_USE_QT2
void javacallMidpStoreEventAndSignalForeground(JavacallMidpEvent evt) {
	MidpEvent mevt;

	mevt.type = evt.type;
	mevt.intParam1 = evt.intParam1;
	mevt.intParam2 = evt.intParam2;
	mevt.intParam3 = evt.intParam3;
	mevt.intParam4 = evt.intParam4;
	mevt.intParam5 = evt.intParam5;

	//midpStoreEventAndSignalForeground(mevt);
	StoreMIDPEventInVmThread(mevt, gForegroundIsolateId);
	//StoreMIDPEventInVmThread(mevt, midpGetAmsIsolateId());
}
#endif
