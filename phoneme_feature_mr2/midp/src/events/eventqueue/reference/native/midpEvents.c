...
#define SET_STRING_EVENT_FIELD(VALUE, STRING, OBJ, ID) \
    if (pcsl_string_utf16_length(&VALUE) >= 0) { \
        midp_kjstring_from_pcsl_string(&VALUE, STRING); \
...
#define GET_STRING_EVENT_FIELD(OBJ, ID, STRING, RESULT) \
    ...
    { \
        pcsl_string __temp; \
        if (PCSL_STRING_OK != midp_kjstring_to_pcsl_string(STRING, &__temp)) { \
 ...
 */
static void
cacheEventFieldIDs(kjobject eventObj, kjclass classObj) {
