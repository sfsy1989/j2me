...
gxpport_copy_area(const jshort *clip, 
    ...
    #ifdef FV_USE_QT4
    QPainter painter;
    painter.drawPixmap(x_dest, y_dest, *qpixmap, x_src, y_src, width, height);
    #else
...
gxpport_draw_rgb(const jshort *clip, 
		 ...
		 REPORT_INFO(LC_LOWUI, "LF:gxpport_draw_rgb\n");
  ...
  QImage image((unsigned char*)rgbData,
                 width, height, scanlen, QImage::Format_ARGB32);
#else
    QImage image((unsigned char*)rgbData,
...
REPORT_INFO(LC_LOWUI, "LF:gxpport_get_displaycolor\n");
...
gxpport_fill_rect(int pixel, const jshort *clip, 
    ...
    REPORT_INFO5(LC_LOWUI, "LF:gxpport_fill_rect(%d, %d, %d, %d)  pixel is 0x%x\n",
		 x, y, width, height,pixel);

	if(dst==NULL){
		REPORT_INFO4(LC_LOWUI, "LF:gxpport_fill_rect-dstIsNull(%d, %d, %d, %d)\n",x, y, width, height);

	}else{
	REPORT_INFO4(LC_LOWUI, "LF:gxpport_fill_rect-dstIsNoNull(%d, %d, %d, %d)\n",x, y, width, height);
	}
	...
	REPORT_INFO(LC_LOWUI, "LF:gxpport_fill_rect-enter\n");
        MScreen * mscreen = qteapp_get_mscreen();
	...
	gc->fillRect(x, y, width, height, gc->brush());
	#else
    mscreen->setupGCParas(-1,pixel,clip,(QPaintDevice*)qpixmap,dotted,x, y, width, height);
    mscreen->setGCType(0);
    mscreen->gcUpdate();
	#endif
    //mscreen->repaint(0,0,640,530);
}
...
int gxpport_get_pixel(int rgb, int gray, int isGray) {
   REPORT_INFO(LC_LOWUI, "LF:gxpport_get_pixel\n");
