...
gx_fill_triangle(int color, const jshort *clip, 
  ...
  fill_triangle(sbuf, GXJ_RGB24TOPIXEL(color), 
		clip, x1, y1, x2, y2, x3, y3);
  fv_add_clip_dirty_rect_xy(sbuf, clip,
                            FV_MIN(FV_MIN(x1, x2), x3),
                            FV_MIN(FV_MIN(y1, y2), y3),
                            FV_MAX(FV_MAX(x1, x2), x3),
                            FV_MAX(FV_MAX(y1, y2), y3));
}
...
gx_copy_area(const jshort *clip, 
  ...
  copy_imageregion(sbuf, sbuf, clip, x_dest, y_dest, width, height,
		   x_src, y_src, 0);
  fv_add_clip_dirty_rect_wh(sbuf, clip, x_dest, y_dest, width, height);
}
        ...
        for (a = x; a < x + width; a++) {
            ...
            if (a < clipX1) {
                ...
                if (!processAlpha || ((value & 0xff000000) == 0xff000000)) {
                    sbuf->pixelData[idx] = GXJ_RGB24TOPIXEL(value);
                } else {
                    unsigned int xA =
                    ...
                    sbuf->pixelData[idx] =
                        GXJ_RGB24TOPIXEL((xA & value) |
                            (XAInv & GXJ_PIXELTORGB24(sbuf->pixelData[idx])));
                }
            }
        } /* loop by rgb data columns */
    } /* loop by rgb data rows */
    fv_add_clip_dirty_rect_wh(sbuf, clip, x, y, width, height);
}
...
gx_get_displaycolor(int color) {
    int newColor = GXJ_PIXELTORGB24(GXJ_RGB24TOPIXEL(color));
              ...
              int x1, int y1, int x2, int y2)
{
  ...
  gxj_pixel_type pixelColor = GXJ_RGB24TOPIXEL(color);
  gxj_screen_buffer screen_buffer;
  gxj_screen_buffer *sbuf = gxj_get_image_screen_buffer_impl(dst, &screen_buffer, NULL);
  sbuf = (gxj_screen_buffer *)getScreenBuffer(sbuf);
...
#ifdef FB_32BPP
#define FAST_PIXEL_SET fast_pixel_set32
void fast_pixel_set32(unsigned * mem, unsigned value, int number_of_pixels)
{
   int i;
...
#else
#define FAST_PIXEL_SET fast_pixel_set
