...
#ifdef FB_32BPP
void unclipped_blit(unsigned int *dstRaster, int dstSpan,
		    unsigned int *srcRaster, int srcSpan,
    ...
    for (i = 0; i < height; i++) {
        ...
        dstRaster = (unsigned int *)(((unsigned char *)dstRaster) + dstSpan);
        srcRaster = (unsigned int *)(((unsigned char *)srcRaster) + srcSpan);
    }
}
