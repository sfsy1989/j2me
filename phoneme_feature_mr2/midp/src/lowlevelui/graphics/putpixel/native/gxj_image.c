  ...
  if (imageSBuf->alphaData == NULL) {
    if (x_dest >= clipX1 && y_dest >= clipY1 &&
      ...
      unclipped_blit(&destSBuf->pixelData[y_dest*destSBuf->width+x_dest],
		    destSBuf->width*sizeof(gxj_pixel_type),
		    &imageSBuf->pixelData[0],
                    imageSBuf->width*sizeof(gxj_pixel_type),
		    imageSBuf->height, 
                    imageSBuf->width*sizeof(gxj_pixel_type),
                    destSBuf);
    } else {
...
#if ENABLE_JPEG
#define OUT_PIXEL_SIZE sizeof(gxj_pixel_type)
/**
 * TBD:
    ...
    if (info) {
        ...
        if (JPEG_To_RGB_decodeHeader(info, inData, inDataLen,
            ...
            if (JPEG_To_RGB_decodeData2(info, outData,
                OUT_PIXEL_SIZE, 0, 0, outDataWidth, outDataHeight) != 0) {
                result = TRUE;
            }
        }
...
clipped_blit(gxj_screen_buffer* dst, int dstX, int dstY, gxj_screen_buffer* src, const jshort *clip) {
  ...
  unclipped_blit(dstRaster, dst->width*sizeof(gxj_pixel_type),
		 srcRaster, src->width*sizeof(gxj_pixel_type),
                  height, width*sizeof(gxj_pixel_type),dst);
}
...
copy_imageregion(gxj_screen_buffer* src, gxj_screen_buffer* dest, const jshort *clip,
    ...
    if (width > 0) {
        ...
        if (src->alphaData != NULL) {
			...
			unsigned char *pbDst = (unsigned char *)pDest; 
                        int a = *pSrcAlpha; 
                        //printf("TODO: alpha blend\n");
                        pbDst[0] = (pbSrc[0] * a + pbDst[0] * (0xff - a)) / 255;
                        pbDst[1] = (pbSrc[1] * a + pbDst[1] * (0xff - a)) / 255;
                        pbDst[2] = (pbSrc[2] * a + pbDst[2] * (0xff - a)) / 255;
                        /**FIXME: alpha  blend max*/
  ...
  if (gxj_get_image_screen_buffer_impl(srcImageDataPtr, &sbuf, NULL) != NULL) {
	...
	for (a = x; a < x + width; a++) {
	  ...
	  rgbBuffer[offset + (a - x) + (b - y) * scanlength] =
	    (alpha << 24) + GXJ_PIXELTORGB24(pixel);
	}
      }
    } else {
      for (b = y; b < y + height; b++) {
	for (a = x; a < x + width; a++) {
	  pixel = sbuf.pixelData[b*sbuf.width + a];
	  rgbBuffer[offset + (a - x) + (b - y) * scanlength] =
	    GXJ_PIXELTORGB24(pixel) | 0xFF000000;
	}
      }
    }
  }
...
void gx_render_image(const java_imagedata * srcImageDataPtr,
  ...
  fv_add_clip_dirty_rect_wh(pdstSBuf, clip, x, y,
                            psrcSBuf->width, psrcSBuf->height);
}
...
extern void gx_render_imageregion(const java_imagedata * srcImageDataPtr,
    ...
    draw_imageregion(psrcSBuf, pdstSBuf,
		     ...
		     x_src, y_src,
		     transform);
    fv_add_clip_dirty_rect_wh(pdstSBuf, clip, x_dest, y_dest,
                              psrcSBuf->width, psrcSBuf->height);
}
