...
/* JPEG file header */
const unsigned char gxutl_jpeg_header[4] = {0xff, 0xd8, 0xff, 0xe1};

/* RAW platform dependent image file header */
const unsigned char gxutl_raw_header[4] = {0x89, 'S', 'U', 'N'};

/* GIF file header */
const unsigned char gxutl_gif_header[3] = {0x47, 0x49, 0x46};
...
MIDP_ERROR gxutl_image_get_info(unsigned char *imgBuf,
	...
	*height = (imgBuf[20] << 24) |
	...
	return MIDP_ERROR_NONE;
               ...
               (memcmp(imgBuf, &gxutl_jpeg_header, 2) == 0)) {
	...
	*format = GXUTL_IMAGE_FORMAT_JPEG;
	...
	/*
        ...
        return MIDP_ERROR_IMAGE_CORRUPTED;
	    	...
	    	(memcmp(imgBuf, &gxutl_gif_header, 3) == 0)) {
		*format = GXUTL_IMAGE_FORMAT_GIF;
		*height = (imgBuf[7]<<8)+imgBuf[6];
		*width = (imgBuf[9]<<8)+imgBuf[8];
        return MIDP_ERROR_NONE; 
    }else {
