 ...
 */
KNIEXPORT void SNI_NewArray(jint type, jint size, kjarray arrayHandle);
 ...
 *     fooArray = new Foo[10];
 */
KNIEXPORT void SNI_NewObjectArray(kjclass elementType, jint size,
                                  kjarray arrayHandle);

/*
 * Creates a reference ID to the given kjobject. The reference ID is a
...
KNIEXPORT jint SNI_AddReference(kjobject objectHandle, jboolean isStrong);
...
/*
 * Returns the kjobject corresponding to the given reference <ref>.
 *
 ...
 * objectHandle.
 */
KNIEXPORT void SNI_GetReference(jint ref, kjobject objectHandle);
 ...
 */
KNIEXPORT void SNI_ReleaseSemaphore(kjobject semaphore);
