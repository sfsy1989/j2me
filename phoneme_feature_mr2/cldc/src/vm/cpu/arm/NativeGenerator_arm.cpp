...
void NativeGenerator::generate_native_string_entries() {
  ...
  //----------------------java.lang.String(StringBuffer)-----------------------
  {
    ...
    const int count_field_offset = value_field_offset + sizeof(kjobject);
...
void NativeGenerator::generate_native_misc_entries() {
...

  {
    ...
    ldr(count,    imm_index(obj, Instance::header_size()+sizeof(kjobject)));
    ...
    str(count, imm_index(obj, Instance::header_size()+sizeof(kjobject)));
 ...
 Segment seg(this, code_segment, 
  ...
  //----------------------java.util.Vector.elementAt---------------------
  {
    ...
    ldr(near_obj, imm_index(obj, Oop::klass_offset()));
    ldr(obj_size, imm_index(obj, Instance::header_size()+sizeof(kjobject)));
    ldr(lock,     imm_index(near_obj, JavaNear::raw_value_offset()));
  ...
  //----------------------java.util.Vector.addElement---------------------
  {
    ...
    ldr(el_count, imm_index(self, Instance::header_size() + sizeof(kjobject)));
#else
    GUARANTEE(Instance::header_size() == Oop::klass_offset() + sizeof(kjobject),
    ...
    str(el_count, imm_index(self, Instance::header_size() + sizeof(kjobject)));
...
bind(label_expand_vector);
    ...
    ldr(new_capacity, imm_index(self, Instance::header_size() + sizeof(kjobject) * 2));
    ...
    str(tmp, imm_index(self, Instance::header_size() + sizeof(kjobject)));
    ...
    if (!ENABLE_ZERO_YOUNG_GENERATION) {
      ...
      add(r0, fill_start, imm(sizeof(kjobject)), ne);
      mov(r1, zero, ne);
      bl(label_memset, ne);
    }
